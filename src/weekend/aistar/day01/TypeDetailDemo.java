...
/*
 * 核心:
 * 1. 自动类型转换 byte->int, char->int,  int->double
 * 2. char涉及到ASCII
 * 类型的强制类型转换[语法没有讲] - 的特殊的场景[byte和int]
 *
 *
 * */
public class TypeDetailDemo {
    ...
    public static void main(String[] args) {
        //java中看到一个  整数  默认是int类型
        //java中看到一个  小数  默认是double类型

        byte b =126;

        // b=> short类型    1是一个int类型
        //大的数据类型和小的数据类型在进行计算的时候,计算的结果是偏向类型大的一方, b + 1的计算结果是int类型
        //byte bb = b + 1;
        ...
        //编译不通过的编译是,最最最主要的是jvm区别对待变量和字面量的
        //如何区别对待的呢?

        //当程序运行都此处的时候,在jvm的"眼里",不关心x具体的值,只关心x的类型,得出的结论是x属于int类型
        //jvm很当然的认为这个x可以在int类型的最小值和最大值之间发生变化
        //就会认为x可能会超出byte类型的范围. 存在"风险" , 为了避免在程序在运行期间发生类型溢出的这种风险
        //强制在编译期间就要去处理 - "强制类型转换"
        //byte by = x;//编译不通过,why???
